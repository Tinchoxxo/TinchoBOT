// © Copyright TinchoHacks | TinchoBOT
// Bot By TinchoHacks
...
const vcard = 'BEGIN:VCARD\n' 
            + 'VERSION:3.0\n' 
            + 'FN:𝕋𝕚𝕟𝕔𝕙𝕠😎🤙🏻\n' 
            + 'ORG:Creador De 𝕋𝕚𝕟𝕔𝕙𝕠𝔹𝕆𝕋-𝕍𝟛\n' 
            + 'TEL;type=CELL;type=VOICE;waid=543513155034:+54 9 351315-5034\n' 
            + 'END:VCARD'
//WhatsApp Conexión
const
	{
...
async function starts() {
	...
	console.log(color('[ BOT]','aqua'), color("BOT BY TINCHOHACKS", "yellow"))
	console.log('>', '[',color('INFO','blue'),']','Suscribete A TinchoHacks...')
	FxBot.on('qr', () => {
	console.log(color('[','white'), color('!','red'), color(']','white'), color(' Sigueme en instagram @tincho_maldonadoo'))
	})
...
┏━━━━━━━━━━━━━━━━━
┠⊷️ *Suscríbete*:
┠⊷️*https://www.youtube.com/channel/UC89XOPdzAtLuUPTUOEmXTdw
┠⊷️ *Grupo Oficial*:
┠⊷️ *Muy Pronto...
┗━━━━━━━━━━━━━━━━━
┏━━━━━━━━━━━━━━━━━
...
//SETTING
       ...
       const kapankah = ['Otro día','Otra semana','Otro mes','Otro año']
		...
		mess = {
			wait: '「 ❗ 」 Cargando, ESPERA UN RATO LOCO!',
			success: '「 ❗ 」 Carga Exitosa',
			nsfwoff: '「 ❗ 」La Funcion De Nsfw No Está Activa!',
			wrongFormat: '「 ❗ 」 No seas pelotud Escribe Bien El Formato',
			waitmusic: '「 ❗ 」 Espera! Estoy buscando tu musica\nEn cuánto la encuentre la envío pa!!',
			waitimg: '「 ❗ 」 Espera! Estoy creando tu imagen!',
			nsfwoff: '「 ❗ 」La Funcion De Nsfw No Está Activa,bobo o que?!',
			...
			error: {
				...
				Iv: '「 ❗ 」Link Invalido BOBO'
			},
			only: {
				group: '「 ❗ 」Este Comando Solo Puede Ser Usado En Grupos',
				admin: '「 ❗ 」No Eres Admin 💩',
				Badmin: '「 ❗ 」 Necesito Ser Admin Para Este Comando :c'
			}
		}
		const isUrl = (url) => {
...
//========================================================================================================================//
		...
		if (isCmd && isBanned) {
        return console.log(color('[ USUÁRIO BANEADO ] Ignorando comando', 'blue'), color(moment.tz('America/Argentina').format('HH:mm:ss'), 'yellow'), color(`${command}`),'DE:', color(pushname))}
...
ranm = getRandom('.mp3')
dtt.length > 800
? reply('¿Queres Escribir La Biblia?')
...
case prefix+ 'tagall2':
				...
				FxBot.updatePresence(from, Presence.composing) 
					...
					FxBot.sendMessage(from, '𝙈𝙚𝙣𝙘𝙞𝙤𝙣 𝘿𝙚 𝙏𝙤𝙙𝙤𝙨'+teks+'×º°”˜`”°º× 𝕋𝕚𝕟𝕔𝕙𝕠𝔹𝕆𝕋 ×º°”˜`”°º×', text, {quoted: mek})
					break
                case prefix+ 'tagall3':
                case prefix+ 'marcar3':
				FxBot.updatePresence(from, Presence.composing) 
...
case prefix+ 'tovid':
    ...
    case prefix+ 'toimg':
			...
			fakethumb(buffer,'×º°”˜`”°º× 𝕋𝕚𝕟𝕔𝕙𝕠𝔹𝕆𝕋 ×º°”˜`”°º×')
			fs.unlinkSync(ran)
			})
			break
case prefix+ 'tourl':
...
case prefix+ 'attp':
						if (args.length < 1) return reply(`Y El Texto??\n> *Ejemplo* : *${prefix}attp* @tincho_maldonadoo`)
...
.then((a) => {
if (Number(filesize) >= 30000) return sendMediaURL(from, thumb, `*DESCARGADOR DE AUDIO BY Tincho*\n\n*🤓Titulo*: ${title}\n*⏭️Extensión*:MP3\n*⚖️Tamaño*:${filesizeF}\n*Link* : ${a.data}\n\n_Para la duración de más del límite se presenta en forma de enlace_`)
const captions = `*DESCARGADOR DE AUDIO BY Tincho*\n\n*🤓Titulo*: ${title}\n*⏭️Extensión*:MP3\n*⚖️Tamaño*:${filesizeF}\n\n_ESPERE ENVIANDO ARCHIVO, NO SPAMES LA CONCHA DE TU MADRE_`
...
reply(mess.mpv)
				...
				axios.get(`https://tinyurl.com/api-create.php?url=${dl_link}`)
				.then((a) => {
				if (Number(filesize) >= 40000) return sendMediaURL(from, thumb, `*DESCARGADOR DE VIDEO BY Tincho*\n\n*🤓Titulo*: ${title}\n*⏭️Extensión*:MP3\n*File⚖️Tamaño*:${filesizeF}\n*Link* : ${a.data}\n\n_Para la duración de más del límite se presenta en forma de enlace_`)
...
case prefix+ 'play':
                  ...
                  try {
                    ...
                    .then((res) => {
                       const { dl_link, thumb, title, filesizeF, filesize } = res
                        axios.get(`https://tinyurl.com/api-create.php?url=${dl_link}`)
                        .then(async (a) => {
                        if (Number(filesize) >= 100000) return sendMediaURL(from, thumb, `*DESCARGADOR DE MUSICA BY Tincho*\n\n*🤓Titulo*: ${title}\n*⏭️Extensión*: MP3\n*⚖️Tamaño*: ${filesizeF}\n*📎Link* : ${a.data}\n\n_ESPERE ENVIANDO ARCHIVO, NO SPAMES LA CONCHA DE TU MADRE_`)
		                        const captions = `*DESCARGADOR DE MUSICA BY Tincho*\n\n*🤓Titulo*: ${title}\n*⏭️Extensión*: MP3\n*⚖️Tamaño*: ${filesizeF}\n*📎Link*: ${a.data}\n\n_ESPERE ENVIANDO ARCHIVO, NO SPAMES LA CONCHA DE TU MADRE_`
                        sendMediaURL(from, thumb, captions)
    ...
    FxBot.sendMessage(from, options, text, { quoted: mek } )
				break
case prefix+ 'creador':
await FxBot.sendMessage(from, {displayname: "Tincho🧙‍♂️", vcard: vcard}, MessageType.contact)
break
